"""Wspólne elementy interfejsu: nagłówek strony i kafelek."""
...
TILE_CLASSES = (
    'w-44 h-44 cursor-pointer items-center justify-center '
...
def tile(icon: str, title: str, target: str | None = None,
         subtitle: str = '', on_click: Callable | None = None) -> ui.card:
    """Kafelek nawigacyjny. Bez `target` i `on_click` jest wyszarzony."""
    enabled = bool(target or on_click)
    card = ui.card().classes(TILE_CLASSES + ('' if enabled else ' opacity-40'))
    with card:
        ui.icon(icon).classes('text-5xl text-blue-4')
